...
- The effective ruleset is the BASE_CONFIG (the four required sections Overview / Behavior / Rejected and Reverted / Change Log, the Behavior anchor regex, the sentinel text "- None on record.", and defaultMode strict) (src/base-config.js:3-13) merged with a per-repo override read from `lint.config.json` in the specs directory, where each field falls back to base when absent (src/config.js:7-24).
...
- `lint(specsDir, effectiveConfig)` enumerates the specs directory, treats every `.md` file except `README.md` as a capability spec, and parses each into `{ path, content, frontmatter, sections }` (src/engine.js:16-30).
...
- Cross-file index-sync check: README index rows and on-disk specs must be in one-to-one correspondence, and each spec's index date must equal its frontmatter `last-updated` (src/checks/index-sync.js:27-58).
